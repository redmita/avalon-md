...
async function addExif(webpSticker, packname, author, categories = [], extra = {}) {
	...
	const json = { 'sticker-pack-id': stickerPackId, 'sticker-pack-name': packname, 'sticker-pack-publisher': author, 'emojis': categories, 'link-group-bot': 'https://cutt.ly/avalon-wa', 'link-script-github': 'https://github.com/redmita/avalon-md', ...extra };
...
async function video2webp(media, sec) {
	...
	while (duration > 0) {
		await new Promise((resolve, reject) => {
			fluent_ffmpeg(tmpFileIn)
				...
				.addOutputOptions([
					...
					"-loop 0",
					"-ss 00:00:00",
					"-t 00:00:"+duration.toString().padStart(2, '0'),
					"-qscale 25",
					"-compression_level 6",
					"-preset default",
					"-an",
		...
		console.log('\x1b[33m%s\x1b[0m', 'reducing sticker duration to '+duration+'s')
	}
